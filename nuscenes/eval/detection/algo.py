...
def accumulate(gt_boxes: EvalBoxes,
    ...
    taken = set()  # Initially no gt bounding box is matched. Taken = set of ((sample_token, gt_idx),(sample_token, gt_idx),...)
    for ind in sortind: # for each pred box
        pred_box = pred_boxes_list[ind]
        min_dist = np.inf
        match_gt_idx = None

        for gt_idx, gt_box in enumerate(gt_boxes[pred_box.sample_token]): # go over every gt_boxes in the sample/key_frame
