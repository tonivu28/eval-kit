...
def load_prediction(result_path: str, max_boxes_per_sample: int, box_cls = DetectionBox, verbose: bool = False) \
    ...
    assert 'pred_boxes' in data, 'Error: No field `pred_boxes` in result file.'
    
    # Deserialize results and get meta data.
    all_results = EvalBoxes.deserialize(data['pred_boxes'], box_cls)
    if 'meta' in data:
        meta = data['meta']
    else: # add dummy meta data
        meta = {}
...
def load_groundtruth(groundtruth_path: str, box_cls = DetectionBox, verbose: bool = False) \
    ...
    if verbose:
        print("Loaded GT from {}. Found detections for {} samples."
...
#     elif eval_split in {'mini_train', 'mini_val', 'mini'}:
...
#         if eval_split == 'mini':
#             if scene_record['name'] in (splits['mini_train']+splits['mini_val']):
#                 sample_tokens.append(sample_token)
#         else:
#             if scene_record['name'] in splits[eval_split]:
#                 sample_tokens.append(sample_token)
